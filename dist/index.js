...
// src/api/BaseApi.ts
var _agentkeepalive = require('agentkeepalive'); var _agentkeepalive2 = _interopRequireDefault(_agentkeepalive);
var _axios = require('axios'); var _axios2 = _interopRequireDefault(_axios);
var _axiosretry = require('axios-retry'); var _axiosretry2 = _interopRequireDefault(_axiosretry);
var _crypto = require('crypto'); var _crypto2 = _interopRequireDefault(_crypto);
var _httpsproxyagent = require('https-proxy-agent'); var _httpsproxyagent2 = _interopRequireDefault(_httpsproxyagent);
...
var State_default = (initialState) => {
  ...
  return {
    ...
    getCookieValue() {
      return _optionalChain([state2, 'access', _ => _.userSessionToken, 'optionalAccess', _2 => _2.tokenId]);
    },
    ...
    getBearerToken() {
      return _optionalChain([state2, 'access', _3 => _3.bearerToken, 'optionalAccess', _4 => _4.access_token]);
    },
    getBearerTokenMeta() {
...
var _adapternodehttp = require('@pollyjs/adapter-node-http'); var _adapternodehttp2 = _interopRequireDefault(_adapternodehttp);
var _core2 = require('@pollyjs/core');
var _persisterfs = require('@pollyjs/persister-fs'); var _persisterfs2 = _interopRequireDefault(_persisterfs);
...
if (process.env.FRODO_MOCK) {
  _core2.Polly.register(_adapternodehttp2.default);
  _core2.Polly.register(_persisterfs2.default);
...
function getFrodoCommand({ state: state2 }) {
  ...
  try {
    ...
    if (!process.argv[1].endsWith("frodo") && !process.argv[1].endsWith("frodo.exe") && !process.argv[1].endsWith("app.js")) {
      cmd = _path2.default.parse(process.argv[1]).name.replace("-", "/") + "/" + getFrodoArgsId({ start: 2, state: state2 });
    } else {
...
function filterRecording(recording) {
  if (_optionalChain([recording, 'access', _5 => _5.request, 'optionalAccess', _6 => _6.headers])) {
  ...
  if (_optionalChain([recording, 'access', _7 => _7.request, 'optionalAccess', _8 => _8.postData, 'optionalAccess', _9 => _9.text])) {
  ...
  if (_optionalChain([recording, 'access', _10 => _10.response, 'optionalAccess', _11 => _11.content, 'optionalAccess', _12 => _12.text])) {
...
function setupPollyForFrodoLib({
  ...
  const polly = new (0, _core2.Polly)("default");
  polly.configure({
    adapters: ["node-http"],
    mode,
    recordIfMissing,
...
_axiosretry2.default.call(void 0, _axios2.default, {
  retries: 3,
...
var transactionId = `frodo-${_crypto.randomUUID.call(void 0, )}`;
var httpAgent;
var httpsAgent;
function getHttpAgent() {
  if (httpAgent)
    return httpAgent;
  httpAgent = new (0, _agentkeepalive2.default)({
    maxSockets,
    maxFreeSockets,
    timeout: timeout2,
    freeSocketTimeout
...
function getHttpsAgent(allowInsecureConnection) {
  ...
  if (httpsProxy) {
    ...
    httpsAgent = _httpsproxyagent2.default.call(void 0, options);
    return httpsAgent;
  }
  httpsAgent = new _agentkeepalive2.default.HttpsAgent({
    ...options,
    maxSockets,
    maxFreeSockets,
    timeout: timeout2,
...
function curlirize(request, state2) {
  curlirize_default(request, (result, err) => {
    if (err) {
      if (_axios2.default.isAxiosError(err)) {
        printMessage({
          message: `${_optionalChain([err, 'access', _13 => _13.response, 'optionalAccess', _14 => _14.status])}${_optionalChain([err, 'access', _15 => _15.response, 'optionalAccess', _16 => _16.data, 'access', _17 => _17["reason"]]) ? " " + _optionalChain([err, 'access', _18 => _18.response, 'optionalAccess', _19 => _19.data, 'access', _20 => _20["reason"]]) : ""}${_optionalChain([err, 'access', _21 => _21.response, 'optionalAccess', _22 => _22.data, 'access', _23 => _23["message"]]) ? " - " + _optionalChain([err, 'access', _24 => _24.response, 'optionalAccess', _25 => _25.data, 'access', _26 => _26["message"]]) : ""}`,
          type: "error",
          state: state2
        });
      } else {
...
function generateAmApi({
  ...
  const request = _axios2.default.create(requestDetails);
...
async function step({
  ...
  const urlString = service || state2.getAuthenticationService() ? _util2.default.format(
...
async function authorize({
  ...
  const authorizeURL = _util2.default.format(authorizeUrlTemplate, amBaseUrl, "");
...
async function accessToken({
  ...
  const accessTokenURL = _util2.default.format(
...
async function getTokenInfo({
  ...
  const accessTokenURL = _util2.default.format(tokenInfoUrlTemplate, amBaseUrl, "");
  const { data } = await generateOauth2Api({
    resource: getApiConfig3(),
    requestOverride: {},
    state: state2
...
async function clientCredentialsGrant({
  ...
  const { data } = await generateOauth2Api({
    ...
    requestOverride,
    state: state2
  }).post(urlString, _qs2.default.stringify(requestBody), { withCredentials: true });
  return data;
}
...
var _replaceall = require('replaceall'); var _replaceall2 = _interopRequireDefault(_replaceall);
...
function getTypedFilename(name2, type, suffix = "json") {
  const slug = _slugify2.default.call(void 0, name2.replace(/^http(s?):\/\//, ""), {
    remove: /[^\w\s$*_+~.()'"!\-@]+/g
  });
...
function getWorkingDirectory({
  ...
  if (state2.getDirectory()) {
    ...
    if (mkdirs && !_fs2.default.existsSync(wd)) {
      debugMessage({
        message: `ExportImportUtils.getWorkingDirectory: creating directory '${wd}'`,
        state: state2
      });
      _fs2.default.mkdirSync(wd, { recursive: true });
    }
  }
  return wd;
}
...
function findFilesByName(fileName, fast = true, path5 = "./") {
  const entries = _fs2.default.readdirSync(path5, {
    encoding: "utf8",
    withFileTypes: true
  });
...
async function readFiles(directory) {
  const items = await _promises.readdir.call(void 0, directory);
  const filePathsNested = await Promise.all(
    items.map(async (entity) => {
      const path5 = _path.join.call(void 0, directory, entity);
      const isDirectory = (await _promises.lstat.call(void 0, path5)).isDirectory();
      if (isDirectory) {
        return readFiles(path5);
      }
      return {
        path: path5,
        content: await _promises.readFile.call(void 0, path5, "utf8")
      };
    })
  );
...
function substituteEnvParams(input, reader) {
  reader.each((key, value) => {
    input = _replaceall2.default.call(void 0, value, `\${${key}}`, input);
  });
  return input;
}
function unSubstituteEnvParams(input, reader) {
  reader.each((key, value) => {
    input = _replaceall2.default.call(void 0, `\${${key}}`, value, input);
  });
  return input;
}
...
var FrodoError = (_class = class extends Error {
  __init() {this.originalErrors = []}
  __init2() {this.isHttpError = false}
...
function validateJs({ javascriptSource }) {
  _esprima.parseScript.call(void 0, javascriptSource);
  return true;
}
...
async function updateScript({
  ...
  try {
    ...
    if (_optionalChain([error, 'access', _27 => _27.response, 'optionalAccess', _28 => _28.status]) === 409) {
      verboseMessage({
...
async function updateOAuth2Client({
  ...
  try {
    ...
    if (_optionalChain([error, 'access', _29 => _29.response, 'optionalAccess', _30 => _30.status]) === 400 && _optionalChain([error, 'access', _31 => _31.response, 'optionalAccess', _32 => _32.data, 'optionalAccess', _33 => _33.message]) === "Invalid attribute specified.") {
      try {
      ...
      if (key.endsWith("Script")) {
        ...
        if (scriptId !== "[Empty]" && !exportData.script[scriptId]) {
          try {
            ...
            if (!(_optionalChain([error, 'access', _34 => _34.response, 'optionalAccess', _35 => _35.status]) === 403 && _optionalChain([error, 'access', _36 => _36.response, 'optionalAccess', _37 => _37.data, 'optionalAccess', _38 => _38.message]) === "This operation is not available in ForgeRock Identity Cloud.")) {
...
async function createJwkRsa() {
  const jwk = await _nodejose2.default.JWK.createKey("RSA", 4096, { alg: "RS256" });
  return jwk.toJSON(true);
}
async function getJwkRsaPublic(jwkJson) {
  const jwk = await _nodejose2.default.JWK.asKey(jwkJson);
...
async function createSignedJwtToken(payload, jwkJson) {
  const key = await _nodejose2.default.JWK.asKey(jwkJson);
  if (typeof payload === "object") {
    payload = JSON.stringify(payload);
  }
  const jwt = await _nodejose2.default.JWS.createSign(
...
async function verifySignedJwtToken(jwt, jwkJson) {
  const jwk = await _nodejose2.default.JWK.asKey(jwkJson);
  const verifyResult = await _nodejose2.default.JWS.createVerify(jwk).verify(jwt);
  return verifyResult;
}
...
async function updateOAuth2TrustedJwtIssuer({
  ...
  try {
    ...
    if (_optionalChain([error, 'access', _39 => _39.response, 'optionalAccess', _40 => _40.status]) === 400 && _optionalChain([error, 'access', _41 => _41.response, 'optionalAccess', _42 => _42.data, 'optionalAccess', _43 => _43.message]) === "Invalid attribute specified.") {
      try {
        const { validAttributes } = error.response.data.detail;
        validAttributes.push("_id");
...
var templateUsernames = JSON.parse(
  _fs2.default.readFileSync(
    _path2.default.resolve(__dirname, "./templates/autoaccess/Usernames.json"),
    "utf8"
  )
);
var templateUserAgents = JSON.parse(
  _fs2.default.readFileSync(
    _path2.default.resolve(__dirname, "./templates/autoaccess/UserAgents.json"),
    "utf8"
  )
);
var templateIpAddresses = JSON.parse(
  _fs2.default.readFileSync(
    _path2.default.resolve(__dirname, "./templates/autoaccess/IPAddresses.json"),
    "utf8"
  )
);
...
function getAccessTokenUrl(state2) {
  ...
  const url2 = new (0, _url.URL)(accessTokenURL);
...
async function importAgent({
  ...
  try {
    ...
    const agentType = _optionalChain([importData, 'access', _44 => _44.agents, 'access', _45 => _45[agentId], 'optionalAccess', _46 => _46._type, 'access', _47 => _47._id]);
    const result = await putAgentByTypeAndId({
      agentType,
      agentId,
      agentData: importData.agents[agentId],
...
async function importIdentityGatewayAgent({
  ...
  try {
    ...
    const agentType = _optionalChain([importData, 'access', _48 => _48.agents, 'access', _49 => _49[agentId], 'optionalAccess', _50 => _50._type, 'access', _51 => _51._id]);
    if (agentType !== "IdentityGatewayAgent")
      throw new FrodoError(
        `Wrong agent type! Expected 'IdentityGatewayAgent' but got '${agentType}'.`
      );
...
async function importJavaAgent({
  ...
  try {
    ...
    const agentType = _optionalChain([importData, 'access', _52 => _52.agents, 'access', _53 => _53[agentId], 'optionalAccess', _54 => _54._type, 'access', _55 => _55._id]);
    if (agentType !== "J2EEAgent")
      throw new FrodoError(
        `Wrong agent type! Expected 'J2EEAgent' but got '${agentType}'.`
      );
...
async function createProvider({
  ...
  if (location === "remote") {
    ...
    urlString = _util2.default.format(
...
async function createCircleOfTrust2({
  ...
  try {
    ...
    if (_optionalChain([createError, 'access', _60 => _60.response, 'optionalAccess', _61 => _61.data, 'optionalAccess', _62 => _62.code]) === 500 && _optionalChain([createError, 'access', _63 => _63.response, 'optionalAccess', _64 => _64.data, 'optionalAccess', _65 => _65.message]) === "Unable to update entity provider's circle of trust") {
      try {
...
async function updateCircleOfTrust2({
  ...
  try {
    ...
    if (_optionalChain([error, 'access', _66 => _66.response, 'optionalAccess', _67 => _67.data, 'optionalAccess', _68 => _68.code]) === 500 && (_optionalChain([error, 'access', _69 => _69.response, 'optionalAccess', _70 => _70.data, 'optionalAccess', _71 => _71.message]) === "Unable to update entity provider's circle of trust" || _optionalChain([error, 'access', _72 => _72.response, 'optionalAccess', _73 => _73.data, 'optionalAccess', _74 => _74.message]) === "An error occurred while updating the COT memberships")) {
      try {
        const response = await updateCircleOfTrust({ cotId, cotData, state: state2 });
        return response || cotData;
      } catch (error2) {
...
${providers.map((it) => it.split("|")[0]).join("\n")}.`,
        ...
        debugMessage({
          message: `Error ${_optionalChain([error, 'access', _75 => _75.response, 'optionalAccess', _76 => _76.status])} creating/updating circle of trust: ${_optionalChain([error, 'access', _77 => _77.response, 'optionalAccess', _78 => _78.data, 'optionalAccess', _79 => _79.message])}`,
          state: state2
        });
        errors.push(error);
      }
        ...
        readConfigEntity({ entityId: configEntity._id, state: state2 }).catch(
          (readConfigEntityError) => {
            ...
            if (
              ...
              printMessage({
                message: _optionalChain([readConfigEntityError, 'access', _80 => _80.response, 'optionalAccess', _81 => _81.data]),
                type: "error",
                state: state2
              });
              printMessage({
...
async function getFeatures2({
  ...
  try {
    ...
    debugMessage({ message: _optionalChain([error, 'access', _82 => _82.response, 'optionalAccess', _83 => _83.data]), state: state2 });
    state2.setFeatures([]);
  }
  return state2.getFeatures();
}
...
var scrypt = _util.promisify.call(void 0, _crypto2.default.scrypt);
...
var DataProtection = class {
  constructor({
    ...
    const masterKeyPath = () => pathToMasterKey || process.env[Constants_default.FRODO_MASTER_KEY_PATH_KEY] || `${_os.homedir.call(void 0, )}/.frodo/masterkey.key`;
    _masterKey.set(this, async () => {
      if (!sessionKey) {
        try {
          ...
          if (!_fs2.default.existsSync(masterKeyPath())) {
            const masterKey = _crypto2.default.randomBytes(32).toString("base64");
            await _fs.promises.writeFile(masterKeyPath(), masterKey);
          }
          return await _fs.promises.readFile(masterKeyPath(), "utf8");
        } catch (err) {
    ...
    _nonce.set(this, () => _crypto2.default.randomBytes(16));
    _salt.set(this, () => _crypto2.default.randomBytes(64));
    _key.set(
      this,
    ...
    _encrypt.set(this, (key, nonce, data, salt) => {
      const cipher = _crypto2.default.createCipheriv("aes-256-gcm", key, nonce);
  ...
  async decrypt(data) {
    ...
    const masterKey = await _masterKey.get(this)();
    const key = await _key.get(this)(masterKey, salt);
    const decipher = _crypto2.default.createDecipheriv("aes-256-gcm", key, nonce);
...
function getConnectionProfilesPath({ state: state2 }) {
  ...
  const profilesPath = state2.getConnectionProfilesPath() || process.env[Constants_default.FRODO_CONNECTION_PROFILES_PATH_KEY] || `${_os2.default.homedir()}/.frodo/${newProfileFilename}`;
  debugMessage({
...
function migrateFromLegacyProfile() {
  try {
    const legacyPath = `${_os2.default.homedir()}/.frodo/${legacyProfileFilename}`;
    const newPath = `${_os2.default.homedir()}/.frodo/${newProfileFilename}`;
    if (!_fs2.default.existsSync(legacyPath) && !_fs2.default.existsSync(newPath)) {
      _fs2.default.writeFileSync(
        newPath,
        JSON.stringify({}, null, fileOptions.indentation)
      );
    } else if (_fs2.default.existsSync(legacyPath) && !_fs2.default.existsSync(newPath)) {
      _fs2.default.copyFileSync(legacyPath, newPath);
      _fs2.default.renameSync(legacyPath, `${legacyPath}.deprecated`);
    }
  } catch (error) {
    throw new FrodoError(
...
async function initConnectionProfiles({ state: state2 }) {
  ...
  try {
    ...
    const folderName = _path2.default.dirname(filename);
    if (!_fs2.default.existsSync(filename)) {
      if (!_fs2.default.existsSync(folderName)) {
        ...
        _fs2.default.mkdirSync(folderName, { recursive: true });
      }
      if (!_fs2.default.existsSync(filename)) {
        debugMessage({
      ...
      const data = _fs2.default.readFileSync(filename, "utf8");
...
async function getConnectionProfileByHost({
  ...
  const filename = getConnectionProfilesPath({ state: state2 });
  if (!_fs2.default.statSync(filename, { throwIfNoEntry: false })) {
    throw new FrodoError(`Connection profiles file ${filename} not found`);
  }
  const connectionsData = JSON.parse(_fs2.default.readFileSync(filename, "utf8"));
...
Specify a sub-string uniquely identifying a single connection profile host URL.`
  ...
  _fs2.default.writeFileSync(filename, JSON.stringify(connectionsData, null, 2));
}
...
function getTokenCachePath({ state: state2 }) {
  ...
  const tokenCachePath = state2.getTokenCachePath() || process.env[Constants_default.FRODO_TOKEN_CACHE_PATH_KEY] || `${_os2.default.homedir()}/.frodo/${tokenCacheFilename}`;
  debugMessage({
...
function initTokenCache({ state: state2 }) {
  try {
    ...
    if (!_fs2.default.existsSync(filename)) {
      ...
      _fs2.default.writeFileSync(filename, stringify(tokenCache));
    }
    debugMessage({
...
function getChecksum(input) {
  return _uuid.v5.call(void 0, input, UUIDV5_NAMESPACE);
}
function getHostKey(state2) {
  return _uuid.v5.call(void 0, state2.getHost(), _uuid.v5.URL);
}
function getRealmKey(state2) {
  return _uuid.v5.call(void 0, "/", UUIDV5_NAMESPACE);
}
function getTypeKey(tokenType) {
  return _uuid.v5.call(void 0, tokenType, UUIDV5_NAMESPACE);
}
function getSubjectKey(tokenType, state2) {
  if (tokenType === "userSession") {
    return _uuid.v5.call(void 0, state2.getUsername(), UUIDV5_NAMESPACE);
  } else if (tokenType === "userBearer") {
    return _uuid.v5.call(void 0, state2.getUsername(), UUIDV5_NAMESPACE);
  } else if (tokenType === "saBearer") {
    return _uuid.v5.call(void 0, state2.getServiceAccountId(), UUIDV5_NAMESPACE);
  }
}
...
function generateSessionKey(tokenType, state2) {
  switch (tokenType) {
    case "userSession":
      return _uuid.v5.call(void 0, state2.getPassword(), UUIDV5_NAMESPACE);
    case "userBearer":
      return _uuid.v5.call(void 0, state2.getPassword(), UUIDV5_NAMESPACE);
    case "saBearer":
      return _uuid.v5.call(void 0, stringify(state2.getServiceAccountJwk()), UUIDV5_NAMESPACE);
    default:
      return null;
  }
}
...
function purge({ state: state2 }) {
  try {
    ...
    _fs2.default.writeFileSync(filename, stringify(purgedCache));
    debugMessage({
...
function flush({ state: state2 }) {
  try {
    ...
    const filename = getTokenCachePath({ state: state2 });
    _fs2.default.writeFileSync(filename, stringify({}));
    debugMessage({
...
async function determineDeploymentType(state2) {
  ...
  switch (deploymentType) {
    ...
    default: {
      ...
      const verifier = encodeBase64Url(_crypto.randomBytes.call(void 0, 32));
      const challenge = encodeBase64Url(
        _crypto.createHash.call(void 0, "sha256").update(verifier).digest()
      );
      ...
      try {
        ...
        if (_optionalChain([e, 'access', _84 => _84.response, 'optionalAccess', _85 => _85.status]) === 302 && _optionalChain([e, 'access', _86 => _86.response, 'access', _87 => _87.headers, 'optionalAccess', _88 => _88.location, 'optionalAccess', _89 => _89.indexOf, 'call', _90 => _90("code=")]) > -1) {
          verboseMessage({
          ...
          try {
            ...
            if (_optionalChain([ex, 'access', _91 => _91.response, 'optionalAccess', _92 => _92.status]) === 302 && _optionalChain([ex, 'access', _93 => _93.response, 'access', _94 => _94.headers, 'optionalAccess', _95 => _95.location, 'optionalAccess', _96 => _96.indexOf, 'call', _97 => _97("code=")]) > -1) {
              adminClientId = forgeopsClientId;
              verboseMessage({
...
async function getAuthCode(redirectURL, codeChallenge, codeChallengeMethod, state2) {
  try {
    ...
    const redirectLocationURL = _optionalChain([response, 'access', _98 => _98.headers, 'optionalAccess', _99 => _99.location]);
...
function createPayload(serviceAccountId, host) {
  ...
  const jti = _uuid.v4.call(void 0, );
...
async function getFreshSaBearerToken({
  ...
  try {
    ...
    const err = error;
    if (err.isHttpError && err.httpErrorText === "invalid_scope" && _optionalChain([err, 'access', _100 => _100.httpDescription, 'optionalAccess', _101 => _101.startsWith, 'call', _102 => _102("Unsupported scope for service account: ")])) {
...
function scheduleAutoRefresh(forceLoginAsUser, autoRefresh, state2) {
  ...
  if (autoRefresh) {
    const expires = state2.getDeploymentType() === Constants_default.CLASSIC_DEPLOYMENT_TYPE_KEY ? _optionalChain([state2, 'access', _103 => _103.getUserSessionTokenMeta, 'call', _104 => _104(), 'optionalAccess', _105 => _105.expires]) : state2.getUseBearerTokenForAmApis() ? _optionalChain([state2, 'access', _106 => _106.getBearerTokenMeta, 'call', _107 => _107(), 'optionalAccess', _108 => _108.expires]) : Math.min(
      _optionalChain([state2, 'access', _109 => _109.getBearerTokenMeta, 'call', _110 => _110(), 'optionalAccess', _111 => _111.expires]),
      _optionalChain([state2, 'access', _112 => _112.getUserSessionTokenMeta, 'call', _113 => _113(), 'optionalAccess', _114 => _114.expires])
    );
...
async function getTokens({
  ...
  try {
    ...
    if (state2.getCookieValue() || state2.getUseBearerTokenForAmApis() && state2.getBearerToken()) {
      if (_optionalChain([state2, 'access', _115 => _115.getBearerTokenMeta, 'call', _116 => _116(), 'optionalAccess', _117 => _117.from_cache])) {
        verboseMessage({ message: `Using cached bearer token.`, state: state2 });
      }
      if (!state2.getUseBearerTokenForAmApis() && _optionalChain([state2, 'access', _118 => _118.getUserSessionTokenMeta, 'call', _119 => _119(), 'optionalAccess', _120 => _120.from_cache])) {
...
async function updateAdminFederationProvider({
  ...
  try {
    ...
    if (_optionalChain([importError, 'access', _121 => _121.response, 'optionalAccess', _122 => _122.status]) === 400 && _optionalChain([importError, 'access', _123 => _123.response, 'optionalAccess', _124 => _124.data, 'optionalAccess', _125 => _125.message]) === "Invalid attribute specified.") {
...
async function checkForUpdates({
  ...
  const updateCount = _optionalChain([updates, 'access', _126 => _126.secrets, 'optionalAccess', _127 => _127.length]) + _optionalChain([updates, 'access', _128 => _128.variables, 'optionalAccess', _129 => _129.length]) || 0;
  if (updateCount > 0) {
    stopProgressIndicator({
      id: indicatorId,
...
async function applyUpdates({
  ...
  try {
    ...
    stopProgressIndicator({
      id: indicatorId,
      message: `Error: ${_optionalChain([error, 'access', _130 => _130.response, 'optionalAccess', _131 => _131.data, 'optionalAccess', _132 => _132.code]) || error} - ${_optionalChain([error, 'access', _133 => _133.response, 'optionalAccess', _134 => _134.data, 'optionalAccess', _135 => _135.message])}`,
      status: "fail",
      state: state2
    });
    return false;
...
async function updateSocialIdentityProvider({
  ...
  try {
    ...
    if (_optionalChain([error, 'access', _136 => _136.response, 'optionalAccess', _137 => _137.status]) === 400 && _optionalChain([error, 'access', _138 => _138.response, 'optionalAccess', _139 => _139.data, 'optionalAccess', _140 => _140.message]) === "Invalid attribute specified.") {
      const { validAttributes } = error.response.data.detail;
      validAttributes.push("_id", "_type");
      for (const attribute of Object.keys(providerData)) {
        if (!validAttributes.includes(attribute)) {
...
async function exportThemes({
  ...
  try {
    ...
    for (const theme of themes) {
      if (!theme._id)
        theme._id = _uuid.v4.call(void 0, );
      updateProgressIndicator({
        id: indicatorId,
...
async function updateCoordinates({
  ...
  if (serverTree === null) {
    try {
      ...
      if (!_axios2.default.isAxiosError(e) || e.response.status !== 404) {
        throw e;
      }
    }
  }
...
async function getSaml2NodeDependencies(nodeObject, allProviders, allCirclesOfTrust, state2) {
  ...
  for (const samlProperty of samlProperties) {
    ...
    if (entity) {
      try {
        ...
        error.message = `Error reading saml2 dependencies: ${_optionalChain([error, 'access', _141 => _141.response, 'optionalAccess', _142 => _142.data, 'optionalAccess', _143 => _143.message]) || error.message}`;
        errors.push(error);
      }
    }
  }
  try {
    ...
    error.message = `Error reading saml2 dependencies: ${_optionalChain([error, 'access', _144 => _144.response, 'optionalAccess', _145 => _145.data, 'optionalAccess', _146 => _146.message]) || error.message}`;
    errors.push(error);
  }
  if (errors.length) {
    const errorMessages = errors.map((error) => error.message).join("\n");
      ...
      if (deps && state2.getDeploymentType() === Constants_default.CLOUD_DEPLOYMENT_TYPE_KEY || state2.getDeploymentType() === Constants_default.FORGEOPS_DEPLOYMENT_TYPE_KEY) {
        if (emailTemplateNodes.includes(nodeType)) {
          try {
            ...
            error.message = `Error reading email template ${nodeObject.emailTemplateName}: ${_optionalChain([error, 'access', _147 => _147.response, 'optionalAccess', _148 => _148.data, 'optionalAccess', _149 => _149.message]) || error.message}`;
            errors.push(error);
          }
        }
      }
    ...
    if (themePromise) {
      ...
      try {
        ...
        for (const themeObject of themePromiseResults) {
          ...
          _optionalChain([themeObject, 'access', _150 => _150.linkedTrees, 'optionalAccess', _151 => _151.includes, 'call', _152 => _152(treeObject._id)]))) {
            if (verbose)
              printMessage({
                message: `
...
`,
    ...
    - ${providerId}`,
        ...
        try {
          ...
          if (_optionalChain([error, 'access', _153 => _153.response, 'optionalAccess', _154 => _154.status]) === 500 && _optionalChain([error, 'access', _155 => _155.response, 'optionalAccess', _156 => _156.data, 'optionalAccess', _157 => _157.message]) === "Unable to update SMS config: Data validation failed for the attribute, Redirect after form post URL") {
    ...
    - ${cotId}`,
        ...
        try {
          ...
          if (_optionalChain([error, 'access', _158 => _158.response, 'optionalAccess', _159 => _159.status]) === 409 || _optionalChain([error, 'access', _160 => _160.response, 'optionalAccess', _161 => _161.status]) === 500) {
            try {
    ...
    - ${newUuid}${reUuid ? "*" : ""} (${nodeType})`,
      ...
      - identityResource: ${innerNodeData["identityResource"]}`,
        ...
        try {
          ...
          if (_optionalChain([nodeImportError, 'access', _162 => _162.response, 'optionalAccess', _163 => _163.status]) === 400 && _optionalChain([nodeImportError, 'access', _164 => _164.response, 'optionalAccess', _165 => _165.data, 'optionalAccess', _166 => _166.message]) === "Data validation failed for the attribute, Script") {
            errors.push(
              new FrodoError(
          ...
          } else if (_optionalChain([nodeImportError, 'access', _167 => _167.response, 'optionalAccess', _168 => _168.status]) === 400 && _optionalChain([nodeImportError, 'access', _169 => _169.response, 'optionalAccess', _170 => _170.data, 'optionalAccess', _171 => _171.message]) === "Invalid attribute specified.") {
    ...
    - ${newUuid}${reUuid ? "*" : ""} (${nodeType})`,
      ...
      - identityResource: ${nodeData["identityResource"]}`,
        ...
        try {
          ...
          if (_optionalChain([nodeImportError, 'access', _172 => _172.response, 'optionalAccess', _173 => _173.status]) === 400 && _optionalChain([nodeImportError, 'access', _174 => _174.response, 'optionalAccess', _175 => _175.data, 'optionalAccess', _176 => _176.message]) === "Data validation failed for the attribute, Script") {
            errors.push(
              new FrodoError(
          ...
          } else if (_optionalChain([nodeImportError, 'access', _177 => _177.response, 'optionalAccess', _178 => _178.status]) === 400 && _optionalChain([nodeImportError, 'access', _179 => _179.response, 'optionalAccess', _180 => _180.data, 'optionalAccess', _181 => _181.message]) === "Invalid attribute specified.") {
            const { validAttributes } = nodeImportError.response.data.detail;
            validAttributes.push("_id");
    ...
    - Done`,
      ...
      if (_optionalChain([importError, 'access', _182 => _182.response, 'optionalAccess', _183 => _183.status]) === 400 && _optionalChain([importError, 'access', _184 => _184.response, 'optionalAccess', _185 => _185.data, 'optionalAccess', _186 => _186.message]) === "Invalid attribute specified.") {
        const { validAttributes } = importError.response.data.detail;
        validAttributes.push("_id");
...
var fileByIdTreeExportResolver = async function(treeId, state2) {
  ...
  try {
    ...
    const jsonData = JSON.parse(_fs2.default.readFileSync(file, "utf8"));
    debugMessage({
      message: `fileByIdTreeExportResolver: resolved '${treeId}' to ${file}`,
      state: state2
    });
    if (_optionalChain([jsonData, 'access', _187 => _187.tree, 'optionalAccess', _188 => _188._id]) === treeId) {
      treeExport = jsonData;
...
function createFileParamTreeExportResolver(file, state2) {
  const fileParamTreeExportResolver = async function(treeId) {
    ...
    try {
      const jsonData = JSON.parse(_fs2.default.readFileSync(file, "utf8"));
      if (_optionalChain([jsonData, 'access', _189 => _189.tree, 'optionalAccess', _190 => _190._id]) === treeId) {
        treeExport = jsonData;
      } else if (jsonData.trees && jsonData.trees[treeId]) {
        treeExport = jsonData.trees[treeId];
      } else {
          ...
          try {
            ...
            nodePromises.push(
              deleteNode({
                ...
                if (_optionalChain([error, 'optionalAccess', _191 => _191.response, 'optionalAccess', _192 => _192.data, 'optionalAccess', _193 => _193.code]) === 500 && error.response.data.message === "Unable to read SMS config: Node did not exist") {
                  status.nodes[containerNode._id] = { status: "success" };
                  if (verbose)
                    printMessage({
                      message: `Deleted ${containerNode._id} (${containerNode["_type"]["_id"]}) from ${journeyId}`,
    ...
    if (id2 === policySetName) {
      try {
        ...
        try {
          ...
          if (_optionalChain([error, 'access', _194 => _194.response, 'optionalAccess', _195 => _195.status]) === 409) {
  ...
  for (const id2 of Object.keys(importData.policyset)) {
    try {
      ...
      try {
        ...
        if (_optionalChain([error, 'access', _196 => _196.response, 'optionalAccess', _197 => _197.status]) === 409) {
          response = await updatePolicySet({ policySetData, state: state2 });
          imported.push(id2);
        } else
          throw error;
  ...
  for (const id2 of Object.keys(importData.policyset)) {
    try {
      ...
      try {
        ...
        if (_optionalChain([error, 'access', _198 => _198.response, 'optionalAccess', _199 => _199.status]) === 409) {
          response = await updatePolicySet({ policySetData, state: state2 });
        } else
          throw error;
      }
...
async function exportResourceType({
  ...
  try {
    ...
    if (_optionalChain([error, 'access', _200 => _200.response, 'optionalAccess', _201 => _201.status]) === 404) {
      throw new FrodoError(
    ...
    if (id2 === resourceTypeUuid) {
      try {
        ...
        try {
          ...
          if (_optionalChain([createError, 'access', _202 => _202.response, 'optionalAccess', _203 => _203.status]) === 409)
    ...
    if (importData.resourcetype[id2].name === resourceTypeName) {
      try {
        ...
        try {
          ...
          if (_optionalChain([createError, 'access', _204 => _204.response, 'optionalAccess', _205 => _205.status]) === 409)
            response = await putResourceType({
              resourceTypeUuid: id2,
              resourceTypeData,
              state: state2
  ...
  for (const id2 of Object.keys(importData.resourcetype)) {
    try {
      ...
      try {
        ...
        if (_optionalChain([createError, 'access', _206 => _206.response, 'optionalAccess', _207 => _207.status]) === 409)
          response = await putResourceType({
            resourceTypeUuid: id2,
            resourceTypeData,
            state: state2
  ...
  for (const id2 of Object.keys(importData.resourcetype)) {
    try {
      ...
      try {
        ...
        if (_optionalChain([createError, 'access', _208 => _208.response, 'optionalAccess', _209 => _209.status]) === 409)
          response.push(
            await putResourceType({
              resourceTypeUuid: id2,
              resourceTypeData,
    ...
    const fullServiceData = await Promise.all(
      serviceList.map(async (listItem) => {
        try {
          ...
          if (!(_optionalChain([error, 'access', _210 => _210.response, 'optionalAccess', _211 => _211.status]) === 403 && _optionalChain([error, 'access', _212 => _212.response, 'optionalAccess', _213 => _213.data, 'optionalAccess', _214 => _214.message]) === "This operation is not available in ForgeRock Identity Cloud.")) {
            const message = _optionalChain([error, 'access', _215 => _215.response, 'optionalAccess', _216 => _216.data, 'optionalAccess', _217 => _217.message]);
            printMessage({
  ...
  try {
    ...
    if (clean) {
      try {
        ...
        if (!(_optionalChain([error, 'access', _218 => _218.response, 'optionalAccess', _219 => _219.status]) === 404 && _optionalChain([error, 'access', _220 => _220.response, 'optionalAccess', _221 => _221.data, 'optionalAccess', _222 => _222.message]) === "Not Found")) {
          throw new FrodoError(
    ...
    const deleted = await Promise.all(
      serviceList.map(async (serviceListItem) => {
        try {
          ...
          if (!(_optionalChain([error, 'access', _223 => _223.response, 'optionalAccess', _224 => _224.status]) === 403 && _optionalChain([error, 'access', _225 => _225.response, 'optionalAccess', _226 => _226.data, 'optionalAccess', _227 => _227.message]) === "This operation is not available in ForgeRock Identity Cloud.")) {
            const message = _optionalChain([error, 'access', _228 => _228.response, 'optionalAccess', _229 => _229.data, 'optionalAccess', _230 => _230.message]);
            printMessage({
...
async function exportFullConfiguration({
  ...
  let saml = await _asyncOptionalChain([(await exportOrImportWithErrorHandling(
    exportSaml2Providers,
    stateObj,
    errors
  )), 'optionalAccess', async _231 => _231.saml]);
  ...
  if (saml) {
    saml.cot = _optionalChain([cotExport, 'optionalAccess', _232 => _232.saml, 'access', _233 => _233.cot]);
  } else {
    saml = _optionalChain([cotExport, 'optionalAccess', _234 => _234.saml]);
  }
  const fullExport = {
    meta: getMetadata(stateObj),
    agents: await _asyncOptionalChain([(await exportOrImportWithErrorHandling(exportAgents, stateObj, errors)), 'optionalAccess', async _235 => _235.agents]),
    ...
    managedApplication: await _asyncOptionalChain([(await exportOrImportWithErrorHandling(
      ...
      errors
    )), 'optionalAccess', async _241 => _241.managedApplication]),
    policy: await _asyncOptionalChain([(await exportOrImportWithErrorHandling(
      ...
      errors
    )), 'optionalAccess', async _242 => _242.policy]),
    policyset: await _asyncOptionalChain([(await exportOrImportWithErrorHandling(
      ...
      errors
    )), 'optionalAccess', async _243 => _243.policyset]),
    resourcetype: await _asyncOptionalChain([(await exportOrImportWithErrorHandling(
      exportResourceTypes,
      stateObj,
      errors
    )), 'optionalAccess', async _244 => _244.resourcetype]),
    saml,
    script: await _asyncOptionalChain([(await exportOrImportWithErrorHandling(
      ...
      errors
    )), 'optionalAccess', async _245 => _245.script]),
    secrets: await _asyncOptionalChain([(await exportOrImportWithErrorHandling(exportSecrets, stateObj, errors)), 'optionalAccess', async _246 => _246.secrets]),
    service: {
      ...await _asyncOptionalChain([(await exportOrImportWithErrorHandling(
        ...
        errors
      )), 'optionalAccess', async _247 => _247.service]),
      ...await _asyncOptionalChain([(await exportOrImportWithErrorHandling(
        exportServices,
        {
          globalConfig: false,
        ...
        errors
      )), 'optionalAccess', async _248 => _248.service])
    },
    theme: await _asyncOptionalChain([(await exportOrImportWithErrorHandling(exportThemes, stateObj, errors)), 'optionalAccess', async _249 => _249.theme]),
    trees: await _asyncOptionalChain([(await exportOrImportWithErrorHandling(
      ...
      errors
    )), 'optionalAccess', async _250 => _250.trees]),
    variables: await _asyncOptionalChain([(await exportOrImportWithErrorHandling(
      ...
      errors
    )), 'optionalAccess', async _251 => _251.variables])
  };
...
async function getRealms({
  ...
  const urlString = _util2.default.format(realmsListURLTemplate, state2.getHost());
...
async function getRealm({
  ...
  const urlString = _util2.default.format(realmURLTemplate, state2.getHost(), realmId);
  const { data } = await generateAmApi({ resource: getApiConfig23(), state: state2 }).get(
    urlString,
    {
      withCredentials: true
...
async function createRealm({
  ...
  const urlString = _util2.default.format(createRealmURLTemplate, state2.getHost());
  const { data } = await generateAmApi({
...
exports.FrodoError = FrodoError; exports.frodo = frodo; exports.state = state;
